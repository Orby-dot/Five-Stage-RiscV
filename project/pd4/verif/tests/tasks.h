...
task check_M;
  ...
  begin : check_M_func
    ...
    if(
      pc != dut.core.`M_PC ||
      address != dut.core.`M_ADDRESS ||
      rw != dut.core.`M_RW ||
      size_encoded != dut.core.`M_SIZE_ENCODED ||
      data != dut.core.`M_DATA 
    ) begin
...
task check_W;
  ...
  begin : check_W_func
    ...
    if(
      pc != dut.core.`W_PC ||
      enable != dut.core.`W_ENABLE ||
      destination != dut.core.`W_DESTINATION ||
      data != dut.core.`W_DATA 
    ) begin
